...
bool cargaLista(Apuesta *&, int, int, int, double);
...
bool validarNodo(Caballo *, int);
bool validarNodo(Jugador *, int);
bool validarNodo(Apuesta *, int);
...
bool cargaLista(Apuesta *&apuestas, int idApuesta, int idJugador, int idCaballo, double monto){
	Apuesta *nuevoNodo = new Apuesta();
	nuevoNodo->idApuesta = idApuesta;
	nuevoNodo->idJugador = idJugador;
	nuevoNodo->idCaballo = idCaballo;
	nuevoNodo->montoApuesta = monto;
	Apuesta *aux1 = apuestas;
	Apuesta *aux2 = NULL;
	while(aux1 && aux1->idApuesta < idApuesta){
		aux2 = aux1;
		aux1 = aux1->siguiente;
	}
	if(apuestas == aux1){
...
bool validarNodo(Caballo *caballos, int idBuscado){
	Caballo *actual = caballos;
	bool encontrado = false;
	if(caballos){
		while(caballos && !encontrado){
			if(actual->idCaballo == idBuscado){
				encontrado = true;
			}
			actual = actual->siguiente;
		}
...
bool validarNodo(Jugador *jugadores, int idBuscado){
	Jugador *actual = jugadores;
	bool encontrado = false;
	if(actual){
		while(actual && !encontrado){
			if(actual->idJugador == idBuscado){
				encontrado = true;
			}
			actual = actual->siguiente;
		}
...
bool validarNodo(Apuesta *apuestas, int idBuscado){
	Apuesta *actual = apuestas;
	bool encontrado = false;
	if(actual){
		while(actual && !encontrado){
			if(actual->idApuesta == idBuscado){
				encontrado = true;
			}
			actual = actual->siguiente;
		}
...
void conApuestas(Caballo *&caballos){
	int cantJugadores = 0, opcion, idJugador, idCaballo;
	double monto;
	string nombre;
	char op;
		...
		switch(opcion){
			...
			case 3:
				do{
					...
					//Consulta de Datos antes de poder crear la apuesta
					...
					if(op == 's' || op == 'S'){
						...
						//Creacion de la Apuesta
						
						cout<<"Ingrese el Id Del Jugador que desea apostar: ";cin>>idJugador;
						while(!validarNodo(jugadores, idJugador)){
							cout<<"¡El Id del Jugador ingresado no se ha encontrado!\n";
							cout<<"Ingrese el Id Del Jugador que desea apostar: ";cin>>idJugador;
						}
						
						cout<<"Ingrese el Id del Caballo por el que desea apostar: ";cin>>idCaballo;
						while(!validarNodo(caballos, idCaballo)){
							cout<<"¡El Id del Jugador ingresado no se ha encontrado!\n";
							cout<<"Ingrese el Id Del Jugador que desea apostar: ";cin>>idJugador;
						}
						
						cout<<"Ingrese en el monto a apostar en Gs. : "; cin>>monto;
						while(monto < 1000){
							cout<<"¡La Apuesta mínima es de 1000 Gs.!\n";
							cout<<"Ingrese en el monto a apostar en Gs. : "; cin>>monto;
						}
						
						cargaLista(apuestas, id, idJugador, idCaballo, monto) ? id++ : id = id;
					}	
				}while(!terminar);
